...
function EventIcon({ eventType }: { eventType: BillingHistoryEvent['event_type'] }) {
  ...
  switch (eventType) {
    case 'subscription_created':
      return (
        <svg className={cn(iconClass, 'text-teal')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
        </svg>
      );
    case 'subscription_updated':
      return (
        <svg className={cn(iconClass, 'text-marine')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
        </svg>
      );
    case 'subscription_canceled':
      return (
        <svg className={cn(iconClass, 'text-aubergine')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  if (!events || events.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-marine">Aucune activité pour le moment</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <ul className="space-y-4">
          {events.map((event) => (
            <li key={event.id} className="flex items-start gap-3">
              <div className="mt-0.5 shrink-0 rounded-full bg-sable/50 p-1.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {event.description && (
                  <p className="text-xs text-marine truncate">{event.description}</p>
                )}
