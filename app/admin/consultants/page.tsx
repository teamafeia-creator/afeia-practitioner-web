...
export default function AdminConsultantsPage() {
  ...
  useEffect(() => {
    ...
    async function loadConsultants() {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/consultants?${params.toString()}`, { credentials: 'include' });

        if (!isMounted) return;

        if (!response.ok) {
          showToast.error('Erreur lors du chargement des consultants.');
          setErrorMessage('Erreur de chargement des consultants (détails en logs serveur).');
          setRows([]);
          setTotal(0);
          return;
        }
        ...
        console.error('[admin] loadConsultants error:', err);
        if (!isMounted) return;
        showToast.error('Erreur réseau lors du chargement des consultants.');
        setErrorMessage('Erreur de chargement des consultants.');
        setRows([]);
        setTotal(0);
      } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Consultants"
        subtitle="Identites consultants et statuts."
        actions={
      ...
      <AdminDataTable
        rows={rows}
        isLoading={loading}
        emptyMessage={errorMessage ?? 'Aucun consultant trouve.'}
        columns={[
          {
            key: 'full_name',
            header: 'Consultant',
          ...
          },
          {
            ...
            render: (row) => (
              <Button variant="ghost" size="sm" onClick={() => router.push(`/admin/consultants/${row.id}`)}>
                Voir
              </Button>
            )
          }
