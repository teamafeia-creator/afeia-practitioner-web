...
export default function AdminCircularPage() {
  ...
  const loadCircular = useCallback(async () => {
    ...
    try {
      ...
      showToast.error('Erreur reseau lors du chargement Circular.');
      setRows([]);
      setTotal(0);
    } finally {
      setLoading(false);
  ...
  async function triggerSync(consultantId: string) {
    ...
    try {
      ...
      showToast.error('Erreur reseau lors du sync.');
    } finally {
      setSyncingId(null);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <AdminHeader
        title="Circular"
        subtitle="Consultants actives et synchronisation manuelle."
      />
      ...
      <div className="rounded-lg border border-slate-200 bg-white">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-sm text-slate-500">
            ...
            Aucun consultant active Circular.
          </div>
        ) : (
          <>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="transition hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3">
                          <SyncStatusBadge status={row.last_circular_sync_status} />
                        </td>
